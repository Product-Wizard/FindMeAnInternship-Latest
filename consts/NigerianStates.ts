...
const NigerianStatesAndCapital = Object.freeze([
...
const getStates = () => {
  return NigerianStatesAndCapital.map(item => item.state);
}
const getCapital = () => {
  return NigerianStatesAndCapital.map(item => item.capital);
}


const NigerianStates = {
  NigerianStatesAndCapital,
  getStates,
  getCapital,
}
